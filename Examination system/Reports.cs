...
namespace Examination_system
{
    public partial class Reports : Form
    {
        ...
        {
            sqlConnection1.Open();
            sqlCommand1.CommandText = "select distinct concat(s.Fname,' ',s.Lname) as name , Stu_id from Stud_Course_Exam sce inner join Student s on s.St_id=sce.Stu_id where grade is not null";
            SqlDataReader sdr = sqlCommand1.ExecuteReader();
            while (sdr.Read())
            {
        ...
        private void GetStudentExamR3()
        {
            ...
            sqlConnection1.Open();
            sqlCommand1.CommandText = "select distinct Ex_id from Stud_Course_Exam sce where Stu_id=" + StCbR3.SelectedItem.ToString().Split('.')[0] + " and  sce.grade is not null";
            SqlDataReader sdr = sqlCommand1.ExecuteReader();
            while (sdr.Read())
            {
                ExamCbR3.Items.Add(sdr["Ex_id"].ToString());
            }
            sdr.Close();
            sqlConnection1.Close();
        }
        ...
        private void ExamCbR3_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            ReportParameter rp2 = new ReportParameter("exam_Id", ExamCbR3.SelectedItem.ToString());
        ...
        {
            string c = "select distinct Stu_id ,concat(s.Fname,' ',s.Lname) as name from Student s inner Join Stud_Course_Exam ss on  s.St_id=ss.Stu_id  where ss.grade is not null ";
            sqlConnection1.Open();
            sqlCommand1.CommandText = c;
            SqlDataReader sdr = sqlCommand1.ExecuteReader();
            while (sdr.Read())
